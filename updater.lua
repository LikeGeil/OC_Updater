...
function Funktion.status()
    ...
    gpu.set(x, 6, string.format("   Festplatten Speicher: %.1fkB / %.1fkB%s", disk.spaceUsed() / 1024, disk.spaceTotal() / 1024, string.rep(" ", 35)))
...
function Funktion.verarbeiten()
    ...
    if dateien["truncated"] or not komplett then
        ...
        print("Es werden alle alten Datein ersetzt... Bitte warte 1 Minute!")
        kopieren("/update")
        entfernen("/update")
        os.sleep(20)
        entfernen("/github-liste.txt")
